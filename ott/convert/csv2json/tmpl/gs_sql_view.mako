...
%for i, c in enumerate(csv):
  %if c['id'].strip():
  <% id=c['id'].strip().lower() %>select CONCAT('${id}', '::', ${id}r.route_id) as id, ${id}a.agency_name, ${id}r.* 
...
%for i, c in enumerate(csv):
  %if c['id'].strip():
  <% bid = c['id']; id=bid.lower() %>select CONCAT('${bid}', '::', ${id}s.stop_id) as id, '${bid}' as feed_id, ${id}cs.route_short_names, ${id}cs.route_mode, ${id}cs.route_type, ${id}s.* 
